...
const Likes = ({navigation}: Props) => {
    ...
    const TITLE_MAP = {
        [FILTER.RECEIVED_LIKES]: i18n.t('likes.received-likes'),
        [FILTER.GIVEN_LIKES]: i18n.t('likes.given-likes'),
        [FILTER.HIDDEN]: i18n.t('likes.hidden'),
        [FILTER.BLOCKED]: i18n.t('likes.blocked'),
    };
    ...
    return (
        <View style={{flex: 1, height: height, backgroundColor: colors.background}}>
            <AnimatedHeader
                title={TITLE_MAP[filter] || i18n.t('likes.received-likes')}
                showSearch={false}/>
            {loading &&
                <View style={{
                    zIndex: 1,
                ...
                <View style={{flexDirection: 'row', alignItems: 'center'}}>
                    <Menu
                        ...
                        anchor={<Pressable onPress={() => setMenuFilterVisible(true)}><MaterialCommunityIcons
                            name="dots-vertical" size={24} color={'white'} style={{padding: 8}}/></Pressable>}>
                        {filter !== FILTER.RECEIVED_LIKES && <Menu.Item onPress={() => setFilter(FILTER.RECEIVED_LIKES)}
                                                                        title={i18n.t('likes.received-likes')}/>}
                        {filter !== FILTER.GIVEN_LIKES && <Menu.Item onPress={() => setFilter(FILTER.GIVEN_LIKES)}
                                                                     title={i18n.t('likes.given-likes')}/>}
                        {filter !== FILTER.HIDDEN &&
                            <Menu.Item onPress={() => setFilter(FILTER.HIDDEN)} title={i18n.t('likes.hidden')}/>}
                        {filter !== FILTER.BLOCKED &&
                            <Menu.Item onPress={() => setFilter(FILTER.BLOCKED)} title={i18n.t('likes.blocked')}/>}
                    </Menu>
                </View>
            </View>
                ...
                {results && results.length === 0 && loaded && filter === FILTER.RECEIVED_LIKES &&
                    <View style={{
                        ...
                        <Text style={{
                            ...
                            textAlign: 'center'
                        }}>{i18n.t('likes-empty.subtitle')}</Text>
                    </View>
                }
            </VerticalView>
            <Portal>
                <Modal visible={visible} onDismiss={hideModal} contentContainerStyle={containerStyle}>
                    ...
                    <View style={{padding: 12, alignItems: 'center', marginBottom: 24}}>
                        <Image style={{height: 80, width: 80, borderRadius: 500, marginBottom: 12}}
                               source={{uri: likeResult?.user.profilePicture}}></Image>
